...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/Linkershark/ggv2/aio/permission/ip | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
echo -e " [\e[36m•7\e[0m] Cek Limit User"
echo -e " [\e[36m•8\e[0m] Upgrade Xray Version"
echo -e ""
...
5) clear ; bw ; exit ;;
6) clear ; m-tcp ; exit ;;
7) clear ; cek-limit ; exit ;;
8) clear ; upgrade-xray ; exit ;;
0) clear ; menu ; exit ;;
x) exit ;;
